...
# Lấy danh sách domain (chỉ những thư mục có config.sh)
domains=()
for config in "$dir"/*/config.sh; do
    [ -f "$config" ] || continue
    domains+=("$(basename "$(dirname "$config")")")
done

# Kiểm tra nếu không có domain nào thì dừng script luôn
if [ ${#domains[@]} -eq 0 ]; then
    echo "Không tìm thấy domain nào có config.sh. Dừng script."
    exit 1
fi
